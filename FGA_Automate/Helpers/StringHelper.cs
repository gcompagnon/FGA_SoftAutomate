...
namespace FGA.Automate.Helpers
{
    ...
    internal sealed class StringHelper
    {
        ...
        internal static ExtractedInfo ExtractQuotedString(LineInfo line, char quoteChar, bool allowMultiline)
        {
            ...
            if (line.IsEOL())
                throw new Exception("An empty String found and can be parsed like a QuotedString try to use SafeExtractQuotedString");

            if (line.mLine[line.mCurrentPos] != quoteChar)
                throw new Exception("The source string not begins with the quote char: " + quoteChar);
            ...
            while (line.mLineStr != null)
            {
                ...
                else
                {
                    if (allowMultiline == false)
                        throw new Exception("The current field has an UnClosed quoted string. Complete line: " + res.ToString());
            ...
            throw new Exception("The current field has an unclosed quoted string. Complete Filed String: " + res.ToString());
        }
