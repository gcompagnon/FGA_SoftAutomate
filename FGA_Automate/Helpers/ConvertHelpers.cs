    ...
    public sealed class ConvertHelpers
    {
        ...
        static CultureInfo CreateCulture(string decimalSep)
        {
            ...
            else if (decimalSep == ",")
            {
                ...
                throw new Exception("You can only use '.' or ',' as decimal or grup separators");
        ...
        internal static ConverterBase GetDefaultConverter(string fieldName, Type fieldType)
        {
            if (fieldType.IsArray)
            {
                ...
                if (fieldType.GetArrayRank() != 1)
                    throw new Exception("The array field: '" + fieldName + "' has more than one dimension and is not supported by the library.");
#endif

                fieldType = fieldType.GetElementType();

                if (fieldType.IsArray)
                    throw new Exception("The array field: '" + fieldName + "' is a jagged array and is not supported by the library.");
            ...
            throw new Exception("The field: '" + fieldName + "' has the type: " + fieldType.Name + " that is not a system type, so this field need a CustomConverter (check the docs for more info).");
        }
        ...
        internal sealed class DateTimeMultiFormatConverter : ConverterBase
        {
            ...
            private DateTimeMultiFormatConverter(string[] formats)
            {
                for (int i = 0; i < formats.Length; i++)
                {
                    ...
                    {
                        throw new Exception("The format: '" + formats[i] + " is invalid for the DateTime Converter.");
                    }
                }
        ...
        internal sealed class CharConverter : ConverterBase
        {
            ...
            public CharConverter(string format)
            {
                ...
                if (format == "x")
                    ...
                    throw new Exception("The format of the Char Converter must be \"\", \"x\" for lower or \"X\" for upper");
            }
        ...
        public sealed class GuidConverter : ConverterBase
        {
            ...
            public GuidConverter(string format)
            {
                ...
                if (!(format == "N" || format == "D" || format == "B" || format == "P"))
                    throw new Exception("The format of the Guid Converter must be N, D, B or P.");
        ...
        //    {
        //            //throw new Exception("The format of the String Converter can be null or empty.");
